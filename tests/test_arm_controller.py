#!/usr/bin/env python3
"""
Test arm controllers for LIBERO via dynamics prediction.

Adapted from robocasa/tests/test_arm_controller.py. For each controller:
...
Controller models (from robosuite):
  ...
  - JOINT_VELOCITY (absolute): target_vel = scale_action(a)
        [-1,1] -> [-1, 1] rad/s, dq = vel * dt

Run in container:
    micromamba run -n libero python tests/test_arm_controller.py --num_trials 2
"""
...
# robosuite default scale_action parameters
OSC_POS_SCALE = 0.05
JPOS_SCALE = 0.05
JVEL_SCALE = 1.0
...
def _get_env_action_spec(env):
    """Get (action_low, action_high) from env."""
    inner = getattr(env, "env", env)
    action_spec = getattr(inner, "action_spec", None)
    if action_spec is not None and len(action_spec) >= 2:
        return action_spec[0], action_spec[1]
    # Fallback: assume standard bounds
    dim = getattr(inner, "action_spec", None)
    if dim is not None and hasattr(dim[0], "shape"):
        d = dim[0].shape[0]
    else:
        d = 8
    return np.full(d, -1.0), np.full(d, 1.0)


def _get_env_action_dim(env):
    action_low, _ = _get_env_action_spec(env)
    return action_low.shape[0]


def _create_env(task, img_res, controller):
    """Create OffScreenRenderEnv with given controller."""
...
def _get_robot_state(obs, has_joint_pos: bool):
    ...
    if "robot0_base_to_eef_pos" in obs:
        state["base_to_eef_pos"] = np.array(obs["robot0_base_to_eef_pos"], dtype=np.float64)
    else:
        # Panda fixed base: use eef_pos as proxy (base frame often aligned with world)
        state["base_to_eef_pos"] = np.array(obs["robot0_eef_pos"], dtype=np.float64)
...
def _make_action(arm_controller: str, action_low, action_high, trial_idx: int) -> np.ndarray:
    """Generate a deterministic non-zero action."""
...
def predict_state(mid_state: dict, action: np.ndarray, arm_controller: str,
                  n_steps: int) -> dict:
    """Predict robot state after n_steps of constant action."""
    arm_dim = ARM_CONTROLLER_ACTION_DIMS[arm_controller]
...
def check_prediction(mid_state: dict, actual_state: dict, pred_state: dict,
                     arm_controller: str) -> tuple:
    """Compare predicted vs actual state. Returns (passed, detail_msg)."""
    if arm_controller == "cartesian_pose":
    ...
    return direction_ok and magnitude_ok, detail


def test_controller_dynamics(arm_controller: str, task, num_trials: int) -> tuple:
    """Run num_trials dynamics-prediction tests. Returns (passed, failed, details)."""
    controller = ARM_CONTROLLER_MAP[arm_controller]
    env = _create_env(task, 64, controller)
    enable_joint_pos_observable(env)

    action_low, action_high = _get_env_action_spec(env)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Test LIBERO arm controller dynamics prediction")
    ...
    print("=" * 70)
    print("LIBERO arm controller dynamics prediction test")
    print("=" * 70)
    print(f"  task:             libero_10 / {task.problem_folder}")
    ...
    for arm_controller in ARM_CONTROLLER_MAP:
        ...
        passed, failed, details = test_controller_dynamics(
            arm_controller, task, args.num_trials
        )
        total = passed + failed
